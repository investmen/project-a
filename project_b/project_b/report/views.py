...
def report_request_update(request, report_request_id, is_evaluator=None):
    ...
    #user_id = request.user.id
    user_id = 1
    print("is_evaluator", is_evaluator, type(is_evaluator))
    try:
    ...
    # if a GET (or any other method) we'll create a blank form
        ...
        context = {
            #fixme: 0 => user, 1 => evaluator, None => admin
            'is_evaluator' : is_evaluator if isinstance(is_evaluator, int) else 1,
...
from project_b.report.forms import PropertyForm

def get_or_create_property(form):
    pincode = form.cleaned_data['pincode']
    locality = form.cleaned_data['locality']
    name = form.cleaned_data['name']
    try:
...
class PropertyEvaluatorFormView(LoginRequiredMixin, FormView):
    template_name = 'property_evaluator_search.html'
    ...
    def form_valid(self, form):
        ...
        property_id = get_or_create_property(form)
        #return super().form_valid(form)
        return HttpResponseRedirect('/report/property/%s/report_create/' % property_id)

property_evaluator_form_view = PropertyEvaluatorFormView.as_view()

from django.views.generic.edit import CreateView
from project_b.report.models import Report
...
class ReportCreateView(LoginRequiredMixin, CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return HttpResponseRedirect('/report/evaluator/%s/report_list/' % username)
...
class EvaluatorReportListView(LoginRequiredMixin, ListView):

    model = Report
    slug_field = "id"
    slug_url_kwarg = "id"
    template_name = "report/evaluator_report_list.html"

    def get_queryset(self):
        #TODO: check if db index is created on evaluator_id field
        return Report.objects.filter(evaluator=self.kwargs['evaluator_id'])

evaluator_report_list_view = EvaluatorReportListView.as_view()
...
property_form_view = PropertyFormView.as_view()
...
class PropertyReportListView(LoginRequiredMixin, ListView):

    model = Report
    slug_field = "id"
    slug_url_kwarg = "id"
    template_name = "report/property_report_list.html"

    def get_queryset(self):
        #TODO: check if db index is created on property_id field
        return Report.objects.filter(property=self.kwargs['property_id'])

property_report_list_view = PropertyReportListView.as_view()
...
class ReportRequestCreateView(LoginRequiredMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        print("report request created with id: %s" % model.id)
        #TODO: display success message with request id in request_list page
        return HttpResponseRedirect('/report/user/%s/report_request_list/' % username)
        #self.object = None
        #return super().get(request, *args, **kwargs)

report_request_create_view = ReportRequestCreateView.as_view()
